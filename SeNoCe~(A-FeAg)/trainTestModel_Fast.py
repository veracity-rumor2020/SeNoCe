...
def train_test():
	...
	##training & validation
	dfStats=pd.DataFrame(columns=["val_private_vsr_loss", "val_private_vsr_acc"])
	his=model.fit(xtr, ytr, epochs=epoch, batch_size=32, validation_split=0.2, verbose=2)
	statsL=[ his.history["val_private_vsr_loss"], his.history["val_private_vsr_acc"] ]
	#print (statsL)
	dfStats["val_private_vsr_loss"], dfStats["val_private_vsr_acc"]=statsL[0], statsL[1]
